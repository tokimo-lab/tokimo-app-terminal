...
export default function SshDockerPanel({
  ...
  const fetchContainers = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const resp = await terminalApi.dockerPs(terminalId);
  ...
  const fetchImages = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const resp = await terminalApi.dockerImages(terminalId);
  ...
  const fetchNetworks = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const resp = await terminalApi.dockerNetworks(terminalId);
  ...
  const fetchVolumes = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const resp = await terminalApi.dockerVolumes(terminalId);
  ...
  const fetchStats = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const resp = await terminalApi.dockerStats(terminalId);
      setStats(resp.stats);
    } catch {
      setStats([]);
    } finally {
  ...
  const handleViewLogs = useCallback(
    async (containerId: string, name: string) => {
      try {
        const resp = await terminalApi.dockerLogs(terminalId, containerId, 200);
...
function DockerLogTerminal({ logs }: { logs: string }) {
  return (
    <pre className="flex-1 min-h-0 overflow-auto bg-zinc-900/40 p-3 font-mono text-xs whitespace-pre-wrap text-fg-secondary">
      {logs}
    </pre>
  );
}
